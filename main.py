...
def agregar_asterisco(frame, texto):
    label_texto = tk.Label(frame, text=texto)
    label_texto.pack(side=tk.LEFT)
    label_asterisco = tk.Label(frame, text=" *", fg="red")
...
def obtener_datos():
    try:
        ...
        resultado = f"""
        Renta liquida igual o superior a 2.8 veces el valor del arriendo, ${valor_arriendo * 2.8:,.0f}
...
label_nro_est = tk.Label(root, text="N° de estacionamiento:")
...
label_nro_bodega = tk.Label(root, text="N° de bodega:")
...
text_resultado = scrolledtext.ScrolledText(root, width=80, height=20, wrap=tk.WORD)
text_resultado.grid(row=14, column=0, columnspan=2, padx=10, pady=10)
text_resultado.config(state=tk.DISABLED)
